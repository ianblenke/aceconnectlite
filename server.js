...
var server = app.listen(nconf.get('http:port'));
io = io.listen(server);
...
console.log('Server up and listening on port ' + nconf.get('http:port'));
logger.info('Server up and listening on port ' + nconf.get('http:port'));
        ...
        request({
            ...
            function(error, response, data2) {
                if(error){
                    ...
                    logger.info('CallerId: <phone number>');
                    ...
                    ami.action({
                        ...
                        "Exten":data.exten,
                        "UniqueId":"1",
                        "CallerID":"<phone number>", //TODO: hard-coded for now
                        "Timeout":"30000",
                        "ActionID":"1001",
                        "Priority":"1"
                    }, function(err, res) {});					
...
app.post('/login', function (req, res) {
    ...
    login(req.body.username, req.body.password, function(data){
        ...
        if(false){
            ...
            if(statusMap.has(data.data[0].username)){
                data.message = "User is logged in on another computer.";
                res.status(200).json(data);
            }
            else{
                //Adds user to statusMap. 
                //Tracks if user is already logged in elsewhere
                statusMap.set(data.data[0].username, null);
                
                logger.info('Login, setting caToOutgoingChannelMap: ' + data.data[0].username + ', ' + data.data[0].channel);
                console.log('Login, setting caToOutgoingChannelMap: ' + data.data[0].username + ', ' + data.data[0].channel);
				
                // Tracks the outgoing channel for each CA
                caToOutgoingChannelMap.set(data.data[0].username,data.data[0].channel);
                
                // Tracks the incoming extension for each CA
                logger.info('Populating caToIncomingExtensionMap: ' + data.data[0].username + ' => ' + data.data[0].extension);
                caToIncomingExtensionMap.set(data.data[0].username,data.data[0].extension);
				
                // profile included in the token, set to expire in 15 seconds
                var token = jwt.sign(data.data[0], Buffer(nconf.get('jsonwebtoken:secretkey'), nconf.get('jsonwebtoken:encoding')), { expiresIn: "15000" });
                res.status(200).json({token: token});
                
                console.log('Adding extension: ' + 'SIP/' + data.data[0].extension + nconf.get('queues:inbound:name'));
                
                // TODO - Add AMI QueueAdd call here                
                ami.action({
                    "Action":"QueueAdd",
                    "Interface": "SIP/" + data.data[0].extension,
                    "Paused": "true",
                    "Queue": nconf.get('queues:inbound:name')
                }, function(err, res) {});
...
var csocket = cio.connect('http://localhost:'+nconf.get('http:port'), {reconnect: true, query: 'token=' + ctoken});
